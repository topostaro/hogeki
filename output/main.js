...
var $author$project$Main$Memory = F3(
	function (shells, target, angle) {
		return {angle: angle, shells: shells, target: target};
	});
...
var $author$project$Main$barrelLength = 80.0;
var $elm$core$Basics$pow = _Basics_pow;
var $author$project$Main$shellRadious = 10.0;
var $author$project$Main$targetSize = 60.0;
var $author$project$Main$hit = F2(
	function (shell, target) {
		return _Utils_cmp(
			A2($elm$core$Basics$pow, shell.x - target.x, 2) + A2($elm$core$Basics$pow, shell.y - target.y, 2),
			A2($elm$core$Basics$pow, $author$project$Main$shellRadious + $author$project$Main$targetSize, 2)) < 0;
	});
var $author$project$Main$initialVelocityMultiplier = 1 / 55.0;
var $elm$core$Basics$sqrt = _Basics_sqrt;
var $author$project$Main$gravityMultiplier = 1 / 3500;
var $author$project$Main$updateShell = F2(
	function (computer, shell) {
		var w = computer.screen.width;
		var b = computer.screen.bottom;
		var gravity = $elm$core$Basics$sqrt(
			A2($elm$core$Basics$pow, w, 2) + (4 * A2($elm$core$Basics$pow, b, 2))) * $author$project$Main$gravityMultiplier;
		return {vx: shell.vx, vy: shell.vy - gravity, x: shell.x + shell.vx, y: shell.y + shell.vy};
	});
...
var $evancz$elm_playground$Playground$zigzag = F4(
	function (lo, hi, period, time) {
		return lo + ((hi - lo) * $elm$core$Basics$abs(
			(2 * A2($evancz$elm_playground$Playground$toFrac, period, time)) - 1));
	});
var $author$project$Main$update = F2(
	function (computer, memory) {
		...
		var b = computer.screen.bottom;
		var initialVelocity = $elm$core$Basics$sqrt(
			A2($elm$core$Basics$pow, w, 2) + (4 * A2($elm$core$Basics$pow, b, 2))) * $author$project$Main$initialVelocityMultiplier;
		return {
			angle: A2($elm$core$Basics$atan2, mouse.y - b, mouse.x + (w / 2)),
			shells: _Utils_ap(
				mouse.click ? _List_fromArray(
					[
						{
						vx: initialVelocity * $elm$core$Basics$cos(memory.angle),
						vy: initialVelocity * $elm$core$Basics$sin(memory.angle),
						x: (20 - (w / 2)) + ($author$project$Main$barrelLength * $elm$core$Basics$cos(memory.angle)),
						y: (20 + b) + ($author$project$Main$barrelLength * $elm$core$Basics$sin(memory.angle))
					}
					]) : _List_Nil,
				A2(
					$elm$core$List$map,
					$author$project$Main$updateShell(computer),
					memory.shells)),
			target: A3(
				$elm$core$List$foldl,
				$elm$core$Basics$or,
				...
				A2(
					$elm$core$List$map,
					$author$project$Main$hit(memory.target),
					memory.shells)) ? {
				x: A4($evancz$elm_playground$Playground$zigzag, (-w) / 2, w / 2, 2, computer.time),
				y: A4($evancz$elm_playground$Playground$zigzag, -b, b, 2, computer.time)
			} : memory.target
		};
	});
...
var $author$project$Main$viewShell = F2(
	function (_v0, shell) {
		return A3(
			$evancz$elm_playground$Playground$move,
			shell.x,
			shell.y,
			A2($evancz$elm_playground$Playground$circle, $evancz$elm_playground$Playground$blue, $author$project$Main$shellRadious));
	});
...
var $author$project$Main$viewTarget = F2(
	function (_v0, target) {
		return A3(
			$evancz$elm_playground$Playground$move,
			target.x,
			target.y,
			A2($evancz$elm_playground$Playground$square, $evancz$elm_playground$Playground$red, $author$project$Main$targetSize));
	});
...
var $author$project$Main$view = F2(
	function (computer, memory) {
		...
		return _Utils_ap(
			A2(
				$elm$core$List$map,
				$author$project$Main$viewShell(computer),
				memory.shells),
			_List_fromArray(
				[
					A2(
					...
					'(x: ' + ($elm$core$String$fromFloat(mouse.x + (w / 2)) + (', y: ' + ($elm$core$String$fromFloat(mouse.y - b) + ')')))),
					A3(
					$evancz$elm_playground$Playground$move,
					0,
					50,
					A2(
						...
						$elm$core$String$fromFloat(memory.angle))),
					A3(
					$evancz$elm_playground$Playground$move,
					0,
					100,
					A2($evancz$elm_playground$Playground$words, $evancz$elm_playground$Playground$black, 'クリックで発射')),
					A2($author$project$Main$viewTarget, computer, memory.target),
					A3(
					$evancz$elm_playground$Playground$move,
					(((-w) / 2) + 20) + (($author$project$Main$barrelLength / 2) * $elm$core$Basics$cos(memory.angle)),
					(b + 20) + (($author$project$Main$barrelLength / 2) * $elm$core$Basics$sin(memory.angle)),
					A2(
						$evancz$elm_playground$Playground$rotate,
						((memory.angle / (2 * $elm$core$Basics$pi)) * 360) - 90.0,
						A3($evancz$elm_playground$Playground$rectangle, $evancz$elm_playground$Playground$black, 15, $author$project$Main$barrelLength))),
					A3(
					$evancz$elm_playground$Playground$move,
					((-w) / 2) + 20,
					b + 20,
					A2($evancz$elm_playground$Playground$circle, $evancz$elm_playground$Playground$black, 20))
				]));
	});
